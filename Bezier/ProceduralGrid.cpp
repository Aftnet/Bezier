...
//Return the grid's total width and height as the x and z components of a vector
...
float ProceduralGrid::GetPointHeight(float xpos, float zpos) const
{
	...
	w1 = 1.0f - ((tmp1.z - zpos)/m_fTileSize);
	w2 = 1.0f - w1;
	tmpy1 = ((w1*tmp1.y) + (w2*tmp2.y));

	GetVertexAttrib(POSITION,i+1,j,tmp1);
	GetVertexAttrib(POSITION,i+1,j+1,tmp2);
	w1 = 1.0f - ((tmp1.z - zpos)/m_fTileSize);
	w2 = 1.0f - w1;
	tmpy2 = ((w1*tmp1.y) + (w2*tmp2.y));

	GetVertexAttrib(POSITION,i,j,tmp1);
	GetVertexAttrib(POSITION,i+1,j,tmp2);
	w1 = 1.0f - ((xpos - tmp1.x)/m_fTileSize);
	w2 = 1.0f - w1;
	yposition = w1*tmpy1+w2*tmpy2;
...
Vector4 ProceduralGrid::GetPointNormal(float xpos, float zpos) const
{
	...
	w1 = 1.0f - ((tmp1.z - zpos)/m_fTileSize);
	w2 = 1.0f - w1;
	tmpn2 = (w1*tmp1) + (w2*tmp2);
	tmpn2.NormaliseSelf();

	GetVertexAttrib(NORMAL,i,j,tmp1);
	GetVertexAttrib(NORMAL,i+1,j,tmp2);
	w1 = 1.0f - ((xpos - tmp1.x)/m_fTileSize);
	w2 = 1.0f - w1;
	Normal = (w1*tmpn1) + (w2*tmpn2);
...
void ProceduralGrid::RandomNoise(float magnitude, int levels)
{
	GLuint i,j;
	for(i=0;i<m_iHeight;i++)
	{
		for(j=0;j<m_iWidth;j++)
		{
			m_pVertices[j+i*m_iWidth].Pos.y += magnitude*((float(rand()%levels)/(levels-1))-0.5f);
		}
	}
	NormalGen();
	CalculateMedianHeight();
...
void ProceduralGrid::PerlinNoise(float scale, float magnification)
{
	GLuint i,j;
	float NoiseArg[2];
	for(i=0;i<(m_iHeight);i++)
	{
		for(j=0;j<(m_iWidth);j++)
		{
			NoiseArg[0] = magnification * (m_pVertices[j+i*m_iWidth].Pos.x + 0.1f);
			NoiseArg[1] = magnification * (m_pVertices[j+i*m_iWidth].Pos.z + 0.1f);
			m_pVertices[j+i*m_iWidth].Pos.y += scale * float(noise2(NoiseArg));
		}
	}
	NormalGen();
	CalculateMedianHeight();
